...
public sealed class ObjectPool : MonoBehaviour
{
	...
	public static void CreatePool<T>(T prefab, int buffer) where T : Component
	{
		if (!instance.objectLookup.ContainsKey(prefab))
		{
			instance.objectLookup.Add(prefab, new List<Component>());

			T[] tempPool = new T[buffer];
			for(int i = 0; i < buffer; i++)
			{
				tempPool[i] = prefab.Spawn(new Vector2(-1000, -1000));
			}
	...
	public static T Spawn<T>(T prefab, Vector3 position, Quaternion rotation) where T : Component
	{
		if (instance.objectLookup.ContainsKey(prefab))
		{
            ...
            Debug.Log("Failed to look up pooled object for: " + prefab.name);
...
{
	public static void CreatePool<T>(this T prefab, int buffer) where T : Component
	{
		ObjectPool.CreatePool(prefab, buffer);
	}
